"""Python port of `packages/ai/test/google-shared-image-tool-result-routing.test.ts`."""
...
def make_model(api: str, provider: str, model_id: str) -> Model:
    return Model(
        id=model_id,
        name=model_id,
        api=api,
...
def make_context(model: Model) -> Context:
    return Context(
        messages=[
            UserMessage(content="read the files"),
            AssistantMessage(
                content=[
