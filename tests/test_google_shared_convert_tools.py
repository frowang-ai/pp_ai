"""Python port of `packages/ai/test/google-shared-convert-tools.test.ts`."""
...
from pi_ai.types import JsonSchemaConstrainedSampling, Tool


def make_tool(parameters: dict[str, Any]) -> Tool:
    return Tool(name="test_tool", description="A test tool", parameters=parameters)


def first_declaration(result: list[dict[str, Any]] | None) -> dict[str, Any]:
    assert result is not None
    return result[0]["functionDeclarations"][0]


def test_strips_json_schema_meta_keys_when_use_parameters_true():
    tools = [
        make_tool(
            {
    ...
    decl = first_declaration(convert_tools(tools, True))
    ...
    for key in ("$schema", "$id", "$comment", "$defs", "definitions"):
        assert key not in decl["parameters"]
...
def test_does_not_mutate_the_original_tool_parameters_object():
    ...
    convert_tools([make_tool(original_parameters)], True)
...
def test_preserves_schema_in_parameters_json_schema_when_use_parameters_false():
    ...
    decl = first_declaration(convert_tools(tools, False))
...
def test_uses_validated_function_calling_for_strict_tools_on_gemini_3():
    ...
    with pytest.raises(Exception, match='Tool "test_tool" requires JSON-schema constrained sampling'):
